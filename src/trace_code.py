# Pattern 3の正規表現が存在したファイルのパスをjsonファイルに出力するプログラム
# .apply()と.call()の使用箇所を検出し、該当ファイルのパスを記録
# 単一のrepositoryを対象として処理
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Pattern 3: .apply() vs .call()
        patterns3 = {"slow": re.compile(r".+\.apply\("), "fast": re.compile(r".+\.call\(")}

        patterns5 = {
            "slow": re.compile(r"Object\.prototype\.hasOwnProperty\.call\("),
            "fast": re.compile(r"\w+\.hasOwnProperty\("),  # 任意の変数名 .hasOwnProperty( にマッチ
        }

        process_pattern_files(target_project, patterns5, f"{path.RESULTS}/trace/propaty_files.json")
